(function (scope, undefined) {
  ...
  OBJ.initMeshBuffers = function( gl, mesh ){
    // build buffers used across all objects.
    mesh.normalBuffer =_buildBuffer(gl, gl.ARRAY_BUFFER, mesh.vertexNormals,3);
    mesh.textureBuffer=_buildBuffer(gl, gl.ARRAY_BUFFER, mesh.textures, 2);
    mesh.vertexBuffer =_buildBuffer(gl, gl.ARRAY_BUFFER, mesh.vertices, 3);

    // build object indice buffers.
    mesh.indexBuffer = [];
    for (var i = 0; i < mesh.objects; i++)
      mesh.indexBuffer.push (_buildBuffer(gl, gl.ELEMENT_ARRAY_BUFFER, mesh.indices[i], 1));
  }

  OBJ.deleteMeshBuffers = function( gl, mesh ){
    for (var i = 0; i < mesh.objects; i++) {
      gl.deleteBuffer(mesh.normalBuffer[i]);
      gl.deleteBuffer(mesh.textureBuffer[i]);
      gl.deleteBuffer(mesh.vertexBuffer[i]);
      gl.deleteBuffer(mesh.indexBuffer[i]);
    }
  }
})(this);
